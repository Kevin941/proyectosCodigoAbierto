...
import org.icmp4j.platform.linux.LinuxProcessNativeBridge;
import org.icmp4j.platform.linux.MacProcessNativeBridge;
 ...
 * Time: 8:59:11 PM
 */
public class LinuxJnaNativeBridge extends NativeBridge {
  // my attributes
	private static final Logger logger = Logger.getLogger (LinuxJnaNativeBridge.class);

  /**
	   * The NativeBridge interface
	   * Invoked to initialize this object
	   * fall back to <code>LinuxProcessNativeBridge</code> or <code>MacProcessNativeBridge</code>
	   * if JNA lib is missing
	   */
	  @Override
	  public void initialize () {
	    ...
	    try {
		    ...
		    Icmp4jUtil.setNativeBridge(this);
	    }
	    catch (UnsatisfiedLinkError e) {
	    	// could load the dynamic library (probably because it's not in the JNA class path)
	    	e.printStackTrace();
	    	final int osFamilyCode = PlatformUtil.getOsFamilyCode ();
	    	final NativeBridge newBridge;
	    	if (osFamilyCode == OsFamilyCode.MAC) {
	    		newBridge = new MacProcessNativeBridge();
	    	} else {
	    		newBridge = new LinuxProcessNativeBridge();
	    	}
	    	newBridge.initialize();
	    	Icmp4jUtil.setNativeBridge(newBridge);
		}
	  }
